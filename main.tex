...
To help more people use the results of this research, a link to all code used to generate the analysis and figures is provided in the Appendix. The code is as useful and easy-to-read as we had time to make it. In the Discussion we address as many assumptions as possible, alternative assumptions, and the consequences of using those assumptions.
...
Then, to obtain a result that is similar to that of Perkins et al, we can order the vertebrates by the product $ b_n b_l $ in descending order, with $ a=1 $ denoting the vertebrate with the largest relative transmission potential. Then, we remove the vertebrates that have the most transmission potentials, following the example set by Perkins et al. The cumulative effect of removing subsequently larger whole-number percentages of vertebrate hosts is show in Figures \ref{fig:lorenz_2004_iricinus_SA}, \ref{fig:lorenz_2004_itrianguliceps_SA}, \ref{fig:lorenz_2005_itrianguliceps_SA}, \ref{fig:lorenz_2005_iricinus_FV}, \ref{fig:lorenz_2005_itrianguliceps_FV}. In those figures, the cumulative totals for tick aggregation (total numbers of ticks) are plotted next to the cumulative transmission potential of calculated by using \eqref{JohnstoneRobertsonR0Cumulative}. Each curve is found by removing subsequently larger proportions of the vertebrate host populations. The shape of each Lorenz curve is useful as a visual aide for understanding the inequality in transmission potentials, but we use these to calculate GINI coefficients, which are reported in Table \ref{tab:kielderGINI}.
...
To find the GINI coefficient for each subset of data, we calculate the area between each Lorenz curve and the identity function. Since the identity function represents an idealised homogeneous population, the GINI coefficient summarises the total amount of inequality. For all subsets of data analysed, the GINI coefficient for the curve obtained by using \eqref{JohnstoneRobertsonR0Cumulative} indicates greater heterogeneity than that of the Lorenz curve that just describes tick aggregation. Figures \ref{fig:lorenz_2005_iricinus_FV} and \ref{fig:lorenz_2005_itrianguliceps_FV} show that just 1\% of vertebrate hosts were responsible for all transmission potential of field voles in 2005, for both \textit{I.ricinus} and \textit{I.trianguliceps}.
...
\begin{table}[]
	...
	\end{tabular}
	\caption{REWRITE THIS: The GINI coefficient for each of the five subsets of data, for which we have sufficient data, indicates heterogeneity among vertebrates for their co-feeding transmission potential, compared to the just aggregating tick heterogeneity, based on larval burdens collected in the Kielder Forest.}
...
Before we follow the example set by Lloyd-Smith et al \cite{LloydSmith2005}, it will be useful to re-implement parts of their work. In their 2005 paper, used a GWBP to analyse simulate pathogen transmissions and predict the probability that a chain of transmissions becomes extinct, for some pathogen and population with $ R_0, k $ that could be determined.
...
\begin{figure}[h!]
    ...
    \caption{If $ Z \sim \text{Poisson}(v) $, then the resulting Poisson Mixture is a negative binomial distribution, which is the candidate offspring distribution. The probability that a chain of transmissions becomes extinct for a combination of $R_0$ and $ k $, after the introduction of a single infected individual, is found by fixed point iteration over the negative binomial distribution's probability generating function. Each simulation generates a random value for $ v \sim \text{Gamma}(R_0, \frac{R_0}{k})$.}\label{fig:probabilityOfExtinction}
\end{figure}
...
In this project, we have tick burden data. We introduce a method to obtain co-aggregation data from tick burden data, and that $ R_0, k $ can be found using that data. Then that implies that for our purposes, $ v $ is represented in the co-aggregation data already.
...
\begin{figure}[h!]
    ...
    \caption{A toy example of how we find the offspring distribution. Each nymph has its own contact network with larvae. The out degree for each nymph is the number of larvae that it coaggregates with. A stochastic step is applied to the vector $ X $ to determine the number of larvae that become infectious nymphs. $ R_0 $ and $ k $ are then found by fitting the reparameterised negative binomial distribution to $ X_\alpha $.}
...
To calculate goodness of fit, for each distribution and for each subset of data, we used $ AIC_c $ as did the Lloyd-Smith et al paper, which is $ AIC $ but corrected for small datasets. $ AIC_c $ rewards the best-fitting models while penalising models by their number of parameters. In the sense that the negative binomial, Poisson and geometric distributions are models, the negative binomial's fit will be offset by its extra parameter, while the Poisson and geometric distributions will each be rewarded for having just one parameter. REF DETAILS ABOUT PARAMS. 
...
\begin{table}[]
	\begin{tabular}{|l|l|l|lll|}
		...
		\textit{I. trianguliceps}              & \textit{Common shrew}                  & 2004                           & \multicolumn{1}{l|}{\textit{773.634}}                    & \multicolumn{1}{l|}{$\infty$}           & 1811.540                              \\ \hline
		\textit{I. trianguliceps}              & \textit{Common shrew}                  & 2005                           & \multicolumn{1}{l|}{\textit{370.802}}                    & \multicolumn{1}{l|}{$\infty$}           & 939.5                                 \\ \hline
		\textit{I. trianguliceps}              & \textit{Field vole}                    & 2005                           & \multicolumn{1}{l|}{74.768}                     & \multicolumn{1}{l|}{$\infty$}           & \textit{73.748}                                \\ \hline
		\textit{I. ricinus}                    & \textit{Common shrew}                  & 2004                           & \multicolumn{1}{l|}{\textit{73.725}}                     & \multicolumn{1}{l|}{$\infty$}           & 115.538                               \\ \hline
		\textit{I. ricinus}                    & \textit{Field vole}                    & 2005                           & \multicolumn{1}{l|}{182.205}                    & \multicolumn{1}{l|}{\textit{176.696}}            & 471.78                                \\ \hline
	\end{tabular}
\caption{AICc (corrected AIC), fitted to subsets of the Kielder Forest data. The negative binomial distribution is the best fit for 3 subsets of data, while it is nearly the best fit in 2 cases.}
...
With the negative binomial distribution confirmed as the best-fit (or close to the best) for the co-aggregation data, $ X $, we can now fit the reparameterised negative binomial distribution to subsets of the Kielder Forest data, to obtain the sample mean $ m $ and dispersion parameter $ k $.
...
Now that we have estimates for the sample mean of the co-aggregation data, obtained from subsets of the Kielder Forest data, we could obtain a particular estimate for $ R_0 $ based on some $ \alpha $ that we nominate based on \eqref{alphaDef}. Researchers who wish to calculate the probability that a new outbreak becomes extinct would do well to provide estimates about their particular tick species, host species, pathogen and environmental conditions.
...
Since $ m $ is known for each subset of Kielder Forest data, then letting $ \alpha $ vary allows us to see the effect on $ R_0 $, in FIGURE REF. Simply put, the relationship between $ R_0 $ and $ \alpha $ is linear, such that the function has an intercept of $ R_0=0 $ where $\alpha=0 $. The gradient is the sample mean of the co-aggregation data, $ m $. Note that since $ 0 \le \alpha \le 1 $ then this imposes bounds on the value that $ R_0 $ can possibly take.

CHART OF LINES FOR THE EFFECT THAT $ \alpha $ HAS ON $ R_0 $.

\subsection{Estimates for the probability of extinction} \label{subsection_estimatesForTheProbabilityOfExtinction}

Finally, we can use estimates of $ k $ and possible values of $ R_0 $, obtained for each subset of Kielder Forest data, to determine the probability that a chain of transmissions that starts with a single infectious tick eventually becomes extinct. By using the negative binomial distribution's probability generating function REF EQUATION, reparameterised to use $ R_0 $ and $ k $ for the offspring distribution $ X_{\alpha} $, we can use Fixed Point Iteration to approximate the probability of eventual extinction. However, since do not know the actual value of $ R_0 $, we instead graphically show the range of extinction probabilities based on the possible values of $ \alpha $.

We take inspiration from the charts by Lloyd-Smith et al \cite{LloydSmith2005} and reproduce the plot of probabilities of extinction \ref{fig:probabilityOfExtinction} and the plot of simulations where the first generation to reach 100 offspring is an arbitrary benchmark \ref{fig:firstGenerationToReach100Offspring}. However, where Lloyd-Smith et al let $ R_0 $ and $ k $ vary for their probabilities of extinction, we will instead let $ alpha $ vary for each subset of of Kielder Forest data, since we know $ k $ but do not know $ R_0 $. Also, where Lloyd-Smith et al fixed $ R_0 $ and ran multiple simulations for different values of $ k $, we will instead fix $ k $ for each subset of Kielder Forest data while allowing $ \alpha $ to vary.
