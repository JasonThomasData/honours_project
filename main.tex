...
\item Sarah Cowen, Dr Caroline Foster-Guanzon and Cooper Maher for proofreading. 
\end{itemize}
...
MAKE 200 WORDS, TOO LONG

Tick-borne pathogens cause significant health issues for humans and livestock worldwide. Ticks feed on vertebrate hosts, which allows pathogens to spread and frequently cause harmful diseases. Pathogens sometimes infect ticks via co-feeding transmission, where a feeding tick transmits a pathogen directly to another feeding tick.

This project makes use of a branching process technique to determine the probability that a chain of co-feeding transmissions becomes extinct. We used 2004-2005 data from the Kielder Forest in England, which includes two species of hard bodied (\textit{Ixodidae}) ticks and two species of vertebrate host, to estimate probability of extinction for some unspecified pathogen.

This project presents a technique, which we believe is novel, for using commonly-collected tick burden data to find an offspring distribution. This distribution is required to perform a branching process analysis. We use a reparamaterisation of the negative binomial distribution, in combination with numerical techniques and simulations, to find ranges of extinction probabilities based on values that $ R_0 $ can possibly take. We show that in some years in the Kielder Forest data, and if we specify that co-feeding transmission is the only viable transmission route, some pathogens would certainly become extinct, while others would have a chance of persisting dependent on the species of tick, pathogen, species of vertebrate host and environmental conditions.

This project uses a novel combination of Lorenz curves and a recent result for estimating $ R_0 $ in co-feeding transmission, to determine vertebrate hosts’ relative heterogeneity in tick burdens and in co-feeding transmission potential. While much research has cited the 80:20 rule for heterogeneity in tick burdens, this project finds that co-feeding transmission potential for some vertebrate host species is instead a 99:1 rule. The technique presented here is useful since it uses commonly-collected tick burden data and does not require an estimate for $ R_0 $.
...
\textit{THINK ABOUT: SHOULD THE NEXT PARAGRAPH BE IN A DIFFERENT SECTION, WITH TWO BELOW}
...
\textit{THINK ABOUT: SHOULD THE NEXT TWO PARAGRAPHS BE IN A DIFFERENT SECTION}
...
\begin{figure}[h]
	...
	\centering
	\begin{tabular}{ll}
		\includegraphics[width=.5\linewidth,valign=m]{I. ricinus, 2004} & \includegraphics[width=.5\linewidth,valign=m]{I. ricinus, 2005} \\
		\includegraphics[width=.5\linewidth,valign=m]{I. trianguliceps, 2004} & \includegraphics[width=.5\linewidth,valign=m]{I. trianguliceps, 2005} \\
	\end{tabular}
...
\begin{itemize}
    ...
    \item An adult common shrew is larger than the adult field vole \cite{}, which implies the chances of nymphs and larvae co-aggregating in close enough proximity.
...
In this honours project we use tick co-aggregation data, which is not similar to the contact trace datasets that Lloyd-Smith et al used. Before we can find the offspring distribution's parameters $ R_0 $ and $ k $, for use in a branching process analysis, we must introduce a method to derive a transmission network from the co-aggregation data. For our purposes, $ v $ is already represented in the co-aggregation data. We also test the negative binomial distribution is a good fit to the data.
...
To see the effect of $ k $ in each subset of Kielder Forest data, we can instead assume that each subset has an offspring distribution with $ R_0 = 1.5 $. Figure \ref{fig:allCombinations_firstGenerationReach100Offspring} is a re-implementation of Figure \ref{fig:firstGenerationToReach100Offspring}, using exactly the same methodology, but instead of using arbitrarily-chosen values of $ k $ we will use the values of $ k $ found by fitting the negative binomial distribution to each subset of Kielder Forest data. Figure \ref{fig:allCombinations_populationsOfOutbreaksThatWentExtinct} is another iteration that also uses the fitted values of $ k $, but instead shows the total population of outbreaks that became extinct. Taken together, Figures \ref{fig:allCombinations_firstGenerationReach100Offspring}, \ref{fig:allCombinations_populationsOfOutbreaksThatWentExtinct} reveal that if these particular combinations of tick and host species involved a pathogen, or pathogens, that could infect larvae at a rate of $ R_0 = 1.5 $ per infectious nymph, then those combinations that have lower $ k $ will on average reach the arbitrary benchmark in fewer generations, but will have a wider range of total population sizes, including when outbreaks became extinct. This shows the influence of superspreading, where some nymphs co-feed with a disproportionately high number of larvae, leading to explosive growth that still sometimes ends with eventual extinction.
...
\begin{figure}[]
	\begin{mdframed}[backgroundcolor=grey250,rightline=false,leftline=false,topline=false]
		...
		\caption{The total populations of outbreaks that became extinct where $ R_0 = 1.5 $, for all combinations of Kielder Forest data that have $ m \ge 1 $. The values in square brackets are outbreaks' survival rates after 10,000 generations.}
...
The analysis presented here indicates that for some subsets of data collected in surveys in the Kielder Forest, which involve a particular tick species, host species and are also separated by year, then for some unspecified pathogens, there will be cases where co-feeding transmission alone is viable to maintain a pathogen in nature.
